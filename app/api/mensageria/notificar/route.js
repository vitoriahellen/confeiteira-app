...
export async function POST(request) {
  ...
  const destino = tipo === "entrega" ? process.env.ZAPI_NUMERO_INTERNO : pedido.cliente_telefone;
  if (!destino) {
    return NextResponse.json(
      {
        error:
          tipo === "entrega"
            ? "Defina a variável ZAPI_NUMERO_INTERNO no projeto para receber alertas de entrega."
  ...
  if (!resultado.ok) {
    return NextResponse.json(
      { error: resultado.skipped ? "Z-API não configurada no projeto." : "Não foi possível enviar a mensagem agora." },
      { status: 502 }
    );
  }
