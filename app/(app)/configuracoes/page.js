...
function CardMensageria() {
  ...
  useEffect(() => {
    fetch("/api/mensageria/config")
      .then((r) => r.json())
      .then((data) => {
        setWapi((atual) => ({ ...atual, ...data, token: "" }));
        setCarregando(false);
      })
      .catch(() => setCarregando(false));
  }, []);
  ...
  async function salvar() {
    ...
    await fetch("/api/mensageria/config", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(wapi),
    });
    setWapi((atual) => ({ ...atual, token: "" }));
    setSalvando(false);
    setSalvo(true);
  }
  ...
  return (
    <div className="card" style={{ padding: "1.4rem" }}>
      <h3 className="display" style={{ fontSize: "1.05rem", margin: "0 0 0.9rem" }}>Mensageria (WhatsApp via W-API)</h3>

      <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
        <div>
          <label className="label">Instance ID</label>
          <input className="input" value={wapi.instanceId} onChange={(e) => setWapi({ ...wapi, instanceId: e.target.value })} />
        </div>
        <div>
          ...
          <input
            ...
            type="password"
            autoComplete="off"
            placeholder={wapi.tokenDefinido ? "•••••••• (definido — deixe em branco pra manter)" : "Token da instância"}
            value={wapi.token}
            onChange={(e) => setWapi({ ...wapi, token: e.target.value })}
          />
        </div>
        <div>
          ...
          <input
            className="input"
            value={wapi.numeroInterno}
            onChange={(e) => setWapi({ ...wapi, numeroInterno: e.target.value })}
      ...
      <p style={{ fontSize: "0.75rem", color: "var(--ink-soft)", marginTop: "0.8rem" }}>
        O token fica salvo criptografado no banco de dados — depois de salvo, não é mostrado de novo
...
function AbaUsuarios() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.4rem" }}>
      <form onSubmit={handleSubmit} className="card" style={{ padding: "1.6rem", maxWidth: 480, display: "flex", flexDirection: "column", gap: "0.9rem" }}>
      ...
      <div className="card" style={{ padding: "1.4rem", maxWidth: 760, overflowX: "auto" }}>
        <h3 className="display" style={{ fontSize: "1.05rem", margin: "0 0 0.4rem" }}>Usuárias e permissões por módulo</h3>
        <p style={{ fontSize: "0.85rem", color: "var(--ink-soft)", marginBottom: "1rem" }}>
          Escolha quais partes do sistema cada usuária pode acessar. Administradoras sempre têm acesso a tudo.
        </p>
        {carregando ? (
          <p style={{ color: "var(--ink-soft)" }}>Carregando...</p>
        ) : (
              ...
              {usuarios.map((u) => (
                <tr key={u.id} style={{ borderBottom: "1px solid var(--card-border)" }}>
                  <td style={{ padding: "0.5rem" }}>
                    ...
                    <div style={{ color: "var(--ink-soft)", fontSize: "0.78rem" }}>{u.email}</div>
                    <span
                      className="badge"
                      style={{
                        background: u.papel === "admin" ? "var(--purple-bg)" : "var(--sage-bg)",
                        color: u.papel === "admin" ? "var(--purple)" : "var(--sage)",
                      }}
                    >
                  ...
                  <td style={{ padding: "0.5rem" }}>
                    {u.papel !== "admin" && (
                      <button onClick={() => remover(u.id)} className="btn btn-danger" style={{ fontSize: "0.78rem", padding: "0.3rem 0.7rem" }}>
                        Remover
                      </button>
                    )}
                  </td>
